...
if (dms_get_var("hdn_folder_archive") == "confirm")
	{
	...
	// Step through all documents in folder and move them to destination folder.
...
//print $query; exit(0);
	...
	if ($num_rows > 0)
		...
		while($result_data = $dmsdb->getarray($result))
			...
			$query .= "WHERE obj_id='".$result_data['obj_id']."' AND obj_type='0'";
			$dmsdb->query($query);
			}
		}
	
	dms_auditing($HTTP_POST_VARS['hdn_folder_id'],"folder/archive/dest folder id=".$dest_folder);

	//dms_folder_subscriptions($HTTP_POST_VARS['hdn_file_id']);

	dms_message("The contents of the folder has been archived to the selected destination folder.");
	...
	*/
	...
	print "  <form method='post' name='frm_select_dest' action='folder_archive.php'>\r";
	...
	print "  <td colspan='2' align='left'><input type=button name='btn_submit' value='Archive' onclick='check_for_dest();'>";
	print "                               <input type=button name='btn_cancel' value='" . _DMS_CANCEL . "' onclick='location=\"folder_options.php?obj_id=".$obj_id."\";'></td>\r";
	print "</table>\r";
	print "<input type='hidden' name='hdn_folder_archive' value='confirm'>\r";
